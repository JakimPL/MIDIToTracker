...
def test_a_selector_stating_nothing_answers_every_note() -> None:
    ...
    assert all(EVERY_NOTE.covers(Expression(velocity=velocity)) for velocity in (1, 64, 127))


def test_a_selector_answers_the_notes_its_bands_reach() -> None:
    quiet = Selector({Axis.VELOCITY: Band(low=0, high=63)})
    assert quiet.covers(Expression(velocity=63))
    assert not quiet.covers(Expression(velocity=64))
...
def test_a_selector_reads_its_axes_by_the_names_a_manifest_states() -> None:
    ...
    parsed = Selector.model_validate({"velocity": {"low": 100, "high": 127}})
    assert parsed.covers(Expression(velocity=120))
    assert not parsed.covers(Expression(velocity=99))
